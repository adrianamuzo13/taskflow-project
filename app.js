...
window.addEventListener("load", () => {
    ...
    peliculas.forEach((peli) => {
      ...
      if (grid) {
        ...
        tarjeta.innerHTML = `
            ...
            <button class="btn-barra-vista w-[calc(100%+2rem)] -mx-4 -mb-4 mt-auto bg-primary hover:bg-secondary text-white py-4 font-bold uppercase text-[11px] tracking-widest transition-all duration-300 flex justify-center items-center cursor-pointer border-none rounded-b-[inherit]" title="Marcar como vista"> 
                    <span class="icon-check hidden mr-2 justify-center gap-3">✓</span> 
                    <span class="texto-boton w-full text-center">Marcar como vista</span>
            </button>
        `;
  ...
  buscadorInput.addEventListener("input", (e) => {
    ...
    // Ocultar o mostrar las secciones completas (Drama, Animación, etc.)
    const secciones = document.querySelectorAll(
      "section[id]:not(#seccion-vistas)",
    );
    secciones.forEach((seccion) => {
      // Buscamos si hay al menos una película visible dentro de esta sección
      const tienePelisVisibles = Array.from(
        seccion.querySelectorAll(".oscar-card"),
      ).some((t) => !t.classList.contains("hidden"));

      if (!tienePelisVisibles && filtro !== "") {
  ...
  // LOGICA DE LA SECCIÓN DE CURIOSIDADES

  let aportesComunidad = [];

  const inputCuri = document.getElementById("input-curiosidad");
  const btnCuri = document.getElementById("btn-anadir-curiosidad");
  const muro = document.getElementById("muro-curiosidades");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (btn && !btn.classList.contains("visto")) {
      ...
      nuevoItem.className =
        "item-vista mb-2 flex items-center justify-between p-4 bg-white/10 dark:bg-white/5 rounded-xl border border-primary/10 transition-all";
      nuevoItem.innerHTML = `
