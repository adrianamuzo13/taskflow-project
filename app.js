...
window.addEventListener("load", () => {
  ...
  function renderizarMuro() {
    if (!muroElement) return; // SI EL MURO NO EXISTE, SALE DE LA FUNCIÓN
    muroElement.innerHTML = ""; // LIMPIA EL MURO
    muroElement.className = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10";
  ...
  // FUNCIÓN PARA PUBLICAR UN NUEVO APORTE
  function publicarAporte() {
    const textoRaw = inputCuriElement.value.trim();
    if (textoRaw === "") return;

    const textoFormateado = textoRaw.charAt(0).toUpperCase() + textoRaw.slice(1).toLowerCase();
    const nuevoAporte = {
      id: Date.now(),
      title: textoFormateado,
      createdAt: new Date().toLocaleDateString()
    };

    aportesComunidad.push(nuevoAporte);
    inputCuriElement.value = "";
    renderizarMuro();
  }
  ...
  if (inputCuriElement) {
    inputCuriElement.addEventListener("keypress", (e) => {
...
window.eliminarAporte = function(id) {
